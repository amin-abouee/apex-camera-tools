use crate::camera::{CameraModel, CameraWithResolution, YamlCamera};
use crate::util::{undistort_image, InterpolationMethod};
use clap::Args;
...
pub struct UndistortArgs {
    ...
    #[arg(long)]
    pub target_fx: Option<f64>,

    /// Target focal length Y (optional, defaults to source fy)
    #[arg(long)]
    pub target_fy: Option<f64>,
}

pub fn run(args: UndistortArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let undistorted = undistort_image(&img, &model, target_intrinsics, InterpolationMethod::Bilinear)?;
